...
namespace C968_PA.Database
{
    public class Query
    {
        ...
        public static async Task addPart(int partID, string name, decimal price, int stock, int max, int min, string type, string companyName, Nullable<int> machineId, string user)
        {
            ...
            if (companyName == null )
            {
             await addInhouse(partID, name, price, stock, max, min, machineId, user );
            }
            else if (machineId == null)
            {

                await addOutsource(partID, name, price, stock, max, min, companyName, user);
            }
        ...
        #region Outsourced Part Methods
        ...
        #region Associated Parts Methods
